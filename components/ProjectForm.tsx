...
import { ArrowLeft, Target, Plus, Users, Shield, Send, Check, BrainCircuit, Layers, BarChart3, Clock, UserCog } from 'lucide-react';
...
interface ProjectFormProps {
  ...
  /** The role of the current logged-in user. Determines whether step 5 is shown. */
  currentUserRole?: UserRole | string;
  /** The ID of the current logged-in user. Used as auto-facilitatorId for facilitators. */
...
const ProjectForm: React.FC<ProjectFormProps> = ({
  ...
  // Step 5 (admin only): facilitator selection
  const [selectedFacilitatorId, setSelectedFacilitatorId] = useState<string>(
    editingProject?.facilitatorId ? String(editingProject.facilitatorId) : ''
  );
  ...
  // Load active experts when reaching step 4
  React.useEffect(() => {
    if (step === 4) {
      const fetchExperts = async () => {
        setIsLoadingExperts(true);
        try {
          // Server enforces isActive=true and role=experto — no client-side role filter needed
          const activeExperts = await userService.getActiveExperts();
          setAllExperts(activeExperts);
        } catch (err) {
  ...
  // Load active facilitators when reaching step 5 (admin only)
  React.useEffect(() => {
    if (step === 5 && isAdmin) {
      const fetchFacilitators = async () => {
        setIsLoadingFacilitators(true);
        try {
          const activeFacilitators = await userService.getActiveFacilitators();
          setAllFacilitators(activeFacilitators);
        } catch (err) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Facilitators auto-assign themselves; admins choose in step 5
      const facilitatorId = isAdmin ? selectedFacilitatorId : (currentUserId ?? editingProject?.facilitatorId ?? '');
  ...
  // ─── Sidebar steps definition (dynamic based on role) ─────────────────────
  const sidebarSteps = [
    ...
    ...(isAdmin ? [{ n: 5, label: 'Asignación', sub: 'Facilitador' }] : []),
  ];
    ...
    <div className="space-y-8 md:space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-64 bg-slate-900 p-8 lg:p-10 flex flex-row lg:flex-col justify-between lg:justify-start gap-4 lg:space-y-8 overflow-x-auto no-scrollbar">
          {sidebarSteps.map(s => (
            <div key={s.n} className="flex gap-4 items-center group cursor-default shrink-0">
              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center font-black transition-all ${
                step === s.n
        ...
        <form onSubmit={handleSubmit} className="flex-1 p-6 md:p-10 lg:p-16 space-y-8 md:space-y-10">
          ...
          {step === 2 && (
            <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-4" role="group" aria-labelledby="method-label">
                ...
                <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 ${hasStartedRounds ? 'opacity-50 pointer-events-none' : ''}`}>
                  {[
                    { value: 'wideband-delphi' as const, label: 'Wideband Delphi', icon: BrainCircuit, badge: 'Recomendado', desc: 'Valor numérico libre + consenso por convergencia estadística' },
                    { value: 'planning-poker' as const, label: 'Planning Poker', icon: Layers, badge: null, desc: 'Baraja Fibonacci: 0,1,2,3,5,8,13,21,?' },
                    { value: 'three-point' as const, label: 'Estimación Tres Puntos', icon: BarChart3, badge: null, desc: 'Optimista / Más Probable / Pesimista — fórmula PERT' },
                  ].map((m) => {
                    const Icon = m.icon;
                    ...
                    return (
                      <button
                        ...
                        className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 text-left ${isSelected ? 'border-delphi-keppel bg-delphi-keppel/5 text-delphi-keppel shadow-xl shadow-delphi-keppel/5' : 'border-slate-100 bg-white text-slate-400 hover:border-slate-200'}`}
                      >
                        <div className="flex items-center gap-2 w-full justify-between">
                          ...
                          {m.badge && (
                            <span className="px-2 py-1 rounded-lg bg-delphi-celadon/50 text-delphi-keppel text-[9px] font-black uppercase">{m.badge}</span>
                          )}
                        </div>
          ...
          {/* ── STEP 4: Kickoff — Expertos (activos únicamente) ── */}
          {step === 4 && (
            <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-slate-100 flex flex-col items-center gap-6 min-h-[300px]">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] md:text-xs text-slate-500 font-bold mt-1">Selecciona expertos activos según su área de dominio</p>
                </div>
                ...
                {isLoadingExperts ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center py-10 space-y-2 opacity-50">
                    ...
                    <p className="text-xs font-bold text-slate-400">No hay expertos activos registrados</p>
                  </div>
                )}
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                {isAdmin ? (
                  /* Admin goes to step 5 */
                  <button
                    type="button"
                    onClick={() => setStep(5)}
                    disabled={expertIds.length === 0}
                    className="flex-1 sm:flex-none bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Siguiente
                  </button>
                ) : (
                  /* Facilitator submits directly */
                  <button
                    type="submit"
                    disabled={expertIds.length === 0}
          ...
          {/* ── STEP 5: Asignación de Facilitador (admin only) ── */}
          {step === 5 && isAdmin && (
            <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-slate-100 flex flex-col items-center gap-6 min-h-[300px]">
                {/* Header icon */}
                <div className="bg-delphi-giants p-4 rounded-full shadow-lg shadow-delphi-giants/20 -mt-12 mb-2">
                  <UserCog className="w-8 h-8 md:w-10 md:h-10 text-white" />
                </div>
                <div className="text-center">
                  <h4 className="font-black text-slate-900 uppercase tracking-widest text-sm">Asignar Facilitador</h4>
                  <p className="text-[10px] md:text-xs text-slate-500 font-bold mt-1">Selecciona el facilitador responsable del proyecto</p>
                </div>
                ...
                {isLoadingFacilitators ? (
                    ...
                    {allFacilitators.map(facilitator => {
                      ...
                      return (
                        <button
                          ...
                          aria-label={`Seleccionar facilitador ${facilitator.name}`}
                          className={`p-4 rounded-2xl border-2 transition-all flex items-center gap-3 text-left ${
                            isSelected
                              ? 'border-delphi-giants bg-delphi-giants/5 text-delphi-giants'
                        ...
                        >
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-[10px] shrink-0 ${
                            isSelected ? 'bg-delphi-giants text-white' : 'bg-slate-200 text-slate-400'
                          }`}>
                            {isSelected ? <Check className="w-4 h-4" /> : facilitator.name.charAt(0)}
                          </div>
                          <div className="flex-1 min-w-0 space-y-1">
                            <p className="text-[10px] font-black truncate">{facilitator.name}</p>
                            <p className="text-[8px] text-slate-400 font-bold truncate">{facilitator.email}</p>
                            <span className={`inline-block px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-wider ${
                              isSelected ? 'bg-delphi-giants/15 text-delphi-giants' : 'bg-slate-100 text-slate-400'
                            }`}>
                              Facilitador
                            </span>
                          </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center py-10 space-y-2 opacity-50">
                    ...
                    <p className="text-xs font-bold text-slate-400">No hay facilitadores activos registrados</p>
                  </div>
                )}

                <div className="w-full pt-4 border-t border-slate-50 flex justify-between items-center px-2">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                    Seleccionado: <span className="text-delphi-giants">{selectedFacilitatorId ? '1' : '0'}</span>
                  </p>
                  {!selectedFacilitatorId && <p className="text-[8px] text-red-400 font-bold italic">Se requiere seleccionar uno</p>}
                </div>
              </div>

              {/* Info banner */}
              <div className="bg-delphi-giants/5 p-4 md:p-6 rounded-2xl border border-delphi-giants/20 flex items-start gap-4">
                <Shield className="w-5 h-5 md:w-6 md:h-6 text-delphi-giants shrink-0 mt-1" />
                <p className="text-[10px] md:text-xs text-slate-600 font-medium leading-relaxed">
                  El facilitador seleccionado será el responsable de gestionar las rondas de estimación y moderar el proceso de consenso.
                </p>
              </div>

              <div className="flex flex-col sm:flex-row gap-4">
                <button type="button" onClick={() => setStep(4)} className="flex-1 sm:flex-none px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all">Atrás</button>
                <button
                  type="submit"
                  disabled={!selectedFacilitatorId}
                  className="flex-1 sm:flex-none bg-delphi-giants text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-delphi-giants/20 hover:scale-105 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed"
                >
                  <Send className="w-4 h-4" /> Crear Proyecto
                </button>
              </div>
            </div>
          )}
