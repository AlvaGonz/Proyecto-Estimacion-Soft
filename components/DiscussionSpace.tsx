...
const DiscussionSpace: React.FC = () => {
  ...
  const [comments, setComments] = useState([
    { 
      id: 1, 
      expertId: 'E1', 
      expertName: 'Experto #1', 
      time: 'Hace 10 min', 
      content: 'Considero que el riesgo de latencia en la sincronización Redis no ha sido totalmente mitigado. ¿Hemos evaluado el escenario de "cold start" tras un despliegue masivo?', 
      likes: 4,
      isReply: false
    },
    { 
      id: 2, 
      expertId: 'E3', 
      expertName: 'Experto #3', 
      time: 'Hace 2 min', 
      content: 'De acuerdo con E1. Podríamos implementar una estrategia de pre-warming para las keys más críticas de saldos. Esto incrementa mi estimación en unas 4h.', 
      likes: 0,
      isReply: true
    }
  ]);
  ...
  const handleSendComment = () => {
    try {
      ...
      const newComment = {
        id: Date.now(),
        expertId: 'E2', // Mock current user
        expertName: 'Experto #2',
        time: 'Justo ahora',
        content: comment,
        likes: 0,
        isReply: false
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
           ...
           <div className="space-y-10">
              {comments.length === 0 ? (
                ...
                <>
                  {comments.map((c) => (
                    <div key={c.id} className={`flex gap-6 ${c.isReply ? 'ml-12 lg:ml-20' : ''}`}>
                       <div className={`shrink-0 ${c.isReply ? 'w-12 h-12 rounded-2xl bg-delphi-keppel/10 text-delphi-keppel text-lg' : 'w-14 h-14 rounded-3xl bg-delphi-vanilla text-delphi-orange text-xl'} flex items-center justify-center font-black shadow-inner`}>{c.expertId}</div>
                       <div className="flex-1 space-y-3">
                          <div className="flex items-center justify-between">
                             <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{c.expertName} • {c.time}</span>
                             {!c.isReply && <button className="text-slate-300 hover:text-delphi-orange transition-colors"><Flag className="w-4 h-4" /></button>}
                          </div>
                          <div className={`${c.isReply ? 'bg-delphi-keppel/5 border-delphi-keppel/10' : 'bg-slate-50 border-slate-100'} p-6 rounded-[2rem] rounded-tl-none border relative`}>
                             <p className={`${c.isReply ? 'text-slate-700 font-bold' : 'text-slate-600 font-medium'} leading-relaxed`}>
                                {c.content}
                             </p>
                          </div>
                          {!c.isReply && (
                            <div className="flex items-center gap-4 px-2">
                               <button className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-delphi-keppel hover:text-delphi-celadon"><ThumbsUp className="w-4 h-4" /> {c.likes} Apoyos</button>
