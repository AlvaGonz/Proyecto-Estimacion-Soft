...
import { ConvergenceIndicator } from '../../../shared/components/ConvergenceIndicator';
...
export const RoundAnalysisVerdict: React.FC<RoundAnalysisVerdictProps> = ({
  ...
  // Use shared logic if analysis is not yet available but we have stats
  const cv = displayedStats?.coefficientOfVariation || (displayedStats as any)?.cv || 0;
  const localVerdict = analyzeConvergence(cv);
  const finalAnalysis = analysis || (displayedStats ? localVerdict : null);
  ...
  return (
    <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all overflow-hidden min-w-0 ${isAnalyzing ? 'bg-slate-50/50 backdrop-blur-md' : 'bg-white shadow-xl border-white/40'}`}>
      ...
      ) : (finalAnalysis || (isFacilitator && displayedStats)) ? (
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-700">
          <div className="flex items-center justify-between border-b border-slate-100 pb-6">
            <h4 className="text-xl font-black text-slate-900 flex items-center gap-3">
              <Sparkles className="w-6 h-6 text-delphi-keppel" />
              {analysis ? 'Veredicto de Convergencia' : 'Análisis Estático Proyectado'}
            </h4>
            {finalAnalysis && (
              <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm ${finalAnalysis.level === 'Alta' ? 'bg-delphi-keppel text-white' :
                finalAnalysis.level === 'Media' ? 'bg-delphi-vanilla text-delphi-orange border border-delphi-orange/20' : 'bg-delphi-giants text-white shadow-lg shadow-delphi-giants/20'
                }`}>
                {finalAnalysis.level === 'Alta' ? 'Convergencia Máxima' : finalAnalysis.level === 'Media' ? 'Convergencia Parcial' : 'Baja Coherencia'}
              </span>
            )}
          </div>

          {/* New Convergence Indicator Component */}
          {displayedStats && (
            <div className="mb-6">
              <ConvergenceIndicator 
                cv={cv} 
                level={finalAnalysis?.level || 'Baja'} 
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'CV %', val: `${cv.toFixed(1)}%`, highlight: true }
            ].map((item, i) => (
              <div key={i} className="bg-slate-50/50 p-5 rounded-2xl text-center border border-slate-100/50 hover:bg-white hover:shadow-lg transition-all">
                ...
                <p className={`text-lg font-black ${item.highlight ? (cv <= 15 ? 'text-delphi-keppel' : 'text-delphi-orange') : 'text-slate-900'}`}>
                  {item.val || '-'}
                </p>
              </div>
            ))}
          ...
          {finalAnalysis && (
            <div className={`p-6 rounded-[2rem] border-l-[6px] transition-all shadow-md group/card relative overflow-hidden ${
              finalAnalysis.level === 'Alta' ? 'bg-delphi-keppel/5 border-delphi-keppel' : 
              finalAnalysis.level === 'Media' ? 'bg-delphi-vanilla border-delphi-orange' : 'bg-delphi-giants/5 border-delphi-giants'
            }`}>
              <div className="absolute -right-8 -bottom-8 opacity-5 group-hover:scale-110 transition-transform">
                <BrainCircuit className="w-32 h-32" />
              </div>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2">
                 <Target className="w-3 h-3" /> {analysis ? 'Recomendación Estratégica AI' : 'Veredicto de Motor Estadístico'}
              </p>
              <p className="text-sm font-bold text-slate-900 leading-relaxed italic mb-4">
                "{finalAnalysis.level === 'Alta' ? 'El grupo ha alcanzado un consenso robusto. Listo para consolidación.' : 
                  finalAnalysis.level === 'Media' ? 'Alineación moderada. Los expertos divergentes deben fundamentar su posición.' : 
                  'Disparidad crítica detectada. Requiere revisión técnica profunda.'}"
              </p>
              {finalAnalysis.aiInsights && (
                <div className="mb-4 bg-white/50 p-4 rounded-2xl border border-white/80 shadow-sm relative overflow-hidden group/ai">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-delphi-keppel/5 -rotate-45 translate-x-12 translate-y-12" />
                  <p className="text-[11px] text-delphi-keppel font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                    <BrainCircuit className="w-3.5 h-3.5" /> Profundización de IA EstimaPro
                  </p>
              ...
              <p className="text-[12px] font-medium text-slate-700 leading-relaxed relative z-10">
                <span className="font-extrabold text-slate-900">Pasos recomendados:</span> {finalAnalysis.recommendation}
              </p>
            </div>
          )}
